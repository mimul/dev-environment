...
|경로(자원 이름)       |kebab-case + 복수형 |schedules        |호스트명이 kebab-case이기 때문에, 그것과 일관성을 취한다. 또, REST 지향이라고 하는 전제이기 때문에, 자원명의 마지막은 복수형으로 한다|
...
|상태 코드                 |설명                                                                          |GET/HEAD|POST|PUT|패치|DELETE|
|:-----------------------|:----------------------------------------------------------------------------|:-------|:---|:---|:---|:---|
...
특정의 리퀘스트 헤더에서는 품질값(Quality Value / q값)에 0~1까지의 수치를 지정하는 것으로, 클라이언트가 취득하고 싶은 형식이나 인코딩의 우선도를 지정할 수 있다.
...
|Accept-Language | 언어의 우선순위 표시                               |Accept-Language: ko-KR;q=0.9, en-US;q=0.9              | 1 다국어 대응시 사용 |
